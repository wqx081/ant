#include "ant/server/hybrid_clock.h"
...
ant::Status GetClockTime(ntptimeval* timeval) {
...
Status HybridClock::Update(const Timestamp& to_update) {
  ...
  if (PREDICT_TRUE(now.value() > to_update.value())) {
    return Status::OK();
  }
...
ant::Status HybridClock::WalltimeWithError(uint64_t* now_usec, uint64_t* error_usec) {
  if (PREDICT_FALSE(FLAGS_use_mock_wall_clock)) {
