...
class ThreadTest : public AntTest {};
...
void CallAtExitThread(string* s) {
  Thread::current_thread()->CallAtExit(base::Bind(&ExitHandler, s, base::Unretained("hello 1, ")));
  Thread::current_thread()->CallAtExit(base::Bind(&ExitHandler, s, base::Unretained("hello 2")));
}
