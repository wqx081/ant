...
static void AssertShutdown(ant::Thread* thread, const Status* status) {
...
TEST_F(MultiThreadedRpcTest, TestShutdownDuringService) {
  ...
  scoped_refptr<ant::Thread> threads[kNumThreads];
  Status statuses[kNumThreads];
  for (int i = 0; i < kNumThreads; i++) {
    ASSERT_OK(ant::Thread::Create("test", strings::Substitute("t$0", i),
...
TEST_F(MultiThreadedRpcTest, TestShutdownClientWhileCallsPending) {
  ...
  scoped_refptr<ant::Thread> thread;
  Status status;
  ASSERT_OK(ant::Thread::Create("test", "test",
...
TEST_F(MultiThreadedRpcTest, TestBlowOutServiceQueue) {
  ...
  scoped_refptr<ant::Thread> threads[3];
  Status status[3];
...
TEST_F(MultiThreadedRpcTest, TestShutdownWithIncomingConnections) {
  ...
  vector<scoped_refptr<ant::Thread> > threads;
  for (int i = 0; i < 8; i++) {
    scoped_refptr<ant::Thread> new_thread;
    CHECK_OK(ant::Thread::Create("test", strings::Substitute("t$0", i),
  ...
  for (scoped_refptr<ant::Thread>& t : threads) {
