...
Status ThreadPool::SubmitClosure(const base::Closure& task) {
  // TODO: once all uses of boost::bind-based tasks are dead, implement this
  // in a more straight-forward fashion.
  return SubmitFunc(std::bind(&base::Closure::Run, task));
}
...
Status ThreadPool::CreateThreadUnlocked() {
  ...
  Status s = ant::Thread::Create("thread pool", strings::Substitute("$0 [worker]", name_),
