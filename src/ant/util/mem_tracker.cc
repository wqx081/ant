...
static base::subtle::Atomic64 released_memory_since_gc;
...
bool MemTracker::SoftLimitExceeded(double* current_capacity_pct) {
  ...
  if (usage + rand_.Uniform64(limit_ - soft_limit_) > (uint64_t)limit_) {
...
bool MemTracker::GcMemory(int64_t max_consumption) {
  ...
  if (pre_gc_consumption < (uint64_t)max_consumption) {
    return false;
  }
