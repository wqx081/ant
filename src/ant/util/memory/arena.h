...
//#include "ant/base/logging-inl.h"
#include "ant/base/macros.h"
#include "ant/util/alignment.h"
#include "ant/util/locks.h"
#include "ant/util/memory/memory.h"
...
template <> struct ArenaTraits<true> {
  typedef base::subtle::Atomic32 offset_type;
...
class ArenaBase {
 ...
 private:
  ...
  inline Component* AcquireLoadCurrent() {
    if (THREADSAFE) {
      return reinterpret_cast<Component*>(
        base::subtle::Acquire_Load(reinterpret_cast<base::subtle::AtomicWord*>(&current_)));
    } else {
      return current_;
    }
  }
  ...
  inline void ReleaseStoreCurrent(Component* c) {
    if (THREADSAFE) {
      base::subtle::Release_Store(reinterpret_cast<base::subtle::AtomicWord*>(&current_),
                                  reinterpret_cast<base::subtle::AtomicWord>(c));
    } else {
      current_ = c;
    }
  }
...
inline uint8_t *ArenaBase<true>::Component::AllocateBytesAligned(
  ...
  retry:
base::subtle::Atomic32 offset = base::subtle::Acquire_Load(&offset_);

base::subtle::Atomic32 aligned = KUDU_ALIGN_UP(offset, alignment);
base::subtle::Atomic32 new_offset = aligned + size;

  if (PREDICT_TRUE((uint32_t)new_offset <= size_)) {
