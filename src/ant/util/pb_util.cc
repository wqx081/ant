...
// but modified to work with ant's 'faststring' instead of STL strings.
...
#include "ant/base/strings/escaping.h"
#include "ant/base/strings/fastmem.h"
#include "ant/base/strings/substitute.h"

#include "ant/util/coding-inl.h"
#include "ant/util/coding.h"
#include "ant/util/crc32c.h"
...
//using ant::crc::Crc;
using ant::pb_util::internal::SequentialFileFileInputStream;
using ant::pb_util::internal::WritableFileOutputStream;
using std::deque;
using std::endl;
...
std::ostream& operator<< (std::ostream& os, const ant::pb_util::FileState& state) {
...
static const char kPBContainerMagic[] = "antcntr";
static const int kPBContainerMagicLen = 7;
...
Status WritablePBContainerFile::AppendMsgToBuffer(const Message& msg, faststring* buf) {
  ...
  if (version_ >= 2) {
    uint32_t length_checksum = crc32c::Crc32c((const char *)&data_len, sizeof(data_len));
  ...
  if (version_ == 1) {
    data_checksum = crc32c::Crc32c((const char *)dst, cur_offset);
  } else {
    data_checksum = crc32c::Crc32c((const char *)(dst + data_offset), data_len);
  }
