#include "ant/util/spinlock_profiling.h"
...
METRIC_DEFINE_gauge_uint64(server, spinlock_contention_time,
    "Spinlock Contention Time", ant::MetricUnit::kMicroseconds,
...
METRIC_DEFINE_gauge_uint64(server, tcmalloc_contention_time,
    "TCMalloc Contention Time", ant::MetricUnit::kMicroseconds,
...
base::subtle::Atomic64 val;
  char padding[CACHELINE_SIZE * 2 - sizeof(base::subtle::Atomic64)];
} CACHELINE_ALIGNED;
...
// ant namespace so we don't need to qualify everything.
namespace gutil {
void SubmitSpinLockProfileData(const void *contendedlock, int64 wait_cycles) {
  ant::SubmitSpinLockProfileData(contendedlock, wait_cycles);
}
} // namespace gutil
...
void SubmitSpinLockProfileData(const void* contendedlock, int64 wait_cycles) {
  ...
  auto t = ant::Trace::CurrentTrace();
  if (t) {
    t->metrics()->IncrementTcmallocContentionCycles(wait_cycles);
  }
#endif // !defined(__APPLE__)
  base::subtle::NoBarrier_AtomicIncrement(&ant::g_tcmalloc_contention.val, wait_cycles);
}
} // namespace base
